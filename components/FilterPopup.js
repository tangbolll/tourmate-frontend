...
const FilterPopup = ({ visible, onClose, onApply }) => {
  // 필터 상태 관리
  const [selectedGender, setSelectedGender] = useState('여자만');
  const [selectedAge, setSelectedAge] = useState('누구나');
  const [selectedCategories, setSelectedCategories] = useState(['테마파크']);
