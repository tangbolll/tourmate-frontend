...
export default function AccompanyPost() {
    ...
    const currentUserId = "2"; // Changed to "1" for testing host scenario easily
    ...
    const transformAccompanyDetail = (backendData) => {
        ...
        return {
            ...
            isLiked: backendData.likedByCurrentUser || false, // 좋아요 상태 추가
            isClosed: backendData.isClosed || false, // 마감 상태 추가
        };
    };
    ...
    const fetchAccompanyDetail = async (id) => {
        try {
            ...
            const url = `${API_URL}/api/accompany/AccompanyPost?postId=${id}&userId=${currentUserId}`;
            ...
            if (response.ok) {
                ...
                // 좋아요 상태 별도 조회 (선택사항)
                const likeStatusResponse = await fetch(`${API_URL}/api/accompany/${id}/like/status?id=${currentUserId}`);
                ...
                // 내 신청 목록 조회로 신청 상태 확인 (선택사항)
                const myApplicationsResponse = await fetch(`${API_URL}/api/accompany/my-applications?id=${currentUserId}`);
                if (myApplicationsResponse.ok) {
                    const myApplications = await myApplicationsResponse.json();
                    const isApplied = myApplications.some(app => app.id?.toString() === id);
                ...
                // 호스트 여부 확인: 문자열 비교
                const isCurrentUserHost = transformedData.createdBy === currentUserId;
                console.log('👑 호스트 여부:', isCurrentUserHost);
                setIsHost(isCurrentUserHost);
                
                // 신청 상태 확인 - applyMember 배열에 현재 사용자 ID가 있는지 확인
                const hasApplied = transformedData.applymember.includes(currentUserId);
                console.log('📝 신청 여부:', hasApplied);
        ...
        try {
            ...
            if (response.ok) {
                ...
                return {
                    ...
                    nickname: result.userNickname || '내닉네임',
                    time: result.createdAt ? formatTimeAgo(result.createdAt) : '방금 전',
                    content: result.content || '',
                    profileImage: null,
                    isHost: result.isHostComment || false,
                    replies: [],
                    isTemporary: false
                };
            } else {
    ...
    const handleAddReply = async (content) => {
        ...
        try {
            const savedReply = await saveToServer({
                content: content.trim(),
                parentCommentId: parseInt(replyingTo),
                postId: postId
            }, true);
    ...
    const handleLikeToggle = async () => {
        if (!postId || !currentUserId) {
            Alert.alert("오류", "게시글 또는 사용자 정보가 없습니다.");
            return;
        }

        try {
            // 백엔드: @RequestParam Long id 파라미터명 맞춤
            const url = `${API_URL}/api/accompany/${postId}/like?id=${currentUserId}`;
            ...
            if (response.ok) {
                // 백엔드에서 AccompanyLikeResponse { isLiked: boolean, likeCount: int } 반환
                ...
                const errorText = await response.text();
                console.error('❌ 좋아요 토글 실패:', response.status, errorText);
                Alert.alert('오류', '좋아요 처리 중 오류가 발생했습니다.');
            }
        } catch (error) {
            console.error('❌ 좋아요 토글 네트워크 오류:', error);
            Alert.alert('오류', '네트워크 연결 오류로 좋아요를 처리할 수 없습니다.');
        }
    };
    ...
    const toggleApplication = async () => {
        ...
        try {
            ...
            if (applied) {
                // 취소: DELETE 메서드, /apply/cancel 엔드포인트
                url = `${API_URL}/api/accompany/${postId}/apply/cancel?id=${currentUserId}`;
                method = 'DELETE';
            } else {
                // 신청: POST 메서드, /apply 엔드포인트
                url = `${API_URL}/api/accompany/${postId}/apply?id=${currentUserId}`;
                method = 'POST';
            }
            
            console.log(`🌐 동행 ${applied ? '취소' : '신청'} API 호출:`, url);
            ...
            if (response.ok) {
                ...
                const errorText = await response.text();
                console.error(`❌ 동행 ${applied ? '취소' : '신청'} 실패:`, response.status, errorText);
                Alert.alert('오류', errorText || `동행 ${applied ? '취소' : '신청'} 중 오류가 발생했습니다.`);
            }
        } catch (error) {
            console.error(`❌ 동행 ${applied ? '취소' : '신청'} 네트워크 오류:`, error);
            Alert.alert('오류', '네트워크 연결 오류로 동행 신청을 처리할 수 없습니다.');
        }
    };
    ...
    const closeAccompanyPost = async () => {
        ...
        try {
            ...
            if (response.ok) {
                const message = await response.text(); // 성공 메시지 반환
                console.log('✅ 동행 모집 마감 성공:', message);
                setClosed(true);
                setShowAlarmPopupHost(false);
                Alert.alert("성공", "동행 모집이 마감되었습니다.");
            } else {
                const errorText = await response.text();
                console.error('❌ 동행 모집 마감 실패:', response.status, errorText);
                Alert.alert('오류', errorText || '동행 모집 마감 중 오류가 발생했습니다.');
            }
        } catch (error) {
            console.error('❌ 동행 모집 마감 네트워크 오류:', error);
            Alert.alert('오류', '네트워크 연결 오류로 동행 모집을 마감할 수 없습니다.');
        }
    };
    ...
    }, [postId, currentUserId]); // currentUserId를 의존성 배열에 추가하여 호스트 여부 확인이 정확히 되도록 함
    ...
    const handleConfirmClose = () => {
        closeAccompanyPost(); // API 호출
    };

    // 멤버 데이터 (실제 데이터로 변환) - 이 부분은 백엔드 응답에 따라 더 상세하게 매핑해야 합니다.
    const members = postData ? [
        {
            name: postData.createdByName,
            gender: '여', // 실제 데이터에서 가져와야 함 (e.g., from host object)
            age: '22',  // 실제 데이터에서 가져와야 함
            isHost: true,
            tags: ['호스트']
        },
        // postData.member 또는 postData.participants를 활용하여 나머지 참가자 정보 추가
        // 예시:
        ...(postData.member || []).filter(userId => userId !== postData.createdBy).map(userId => ({
            name: `참가자 ${userId}`, // 실제 닉네임 필요
            gender: '미정', // 실제 데이터에서 가져와야 함
            age: '미정', // 실제 데이터에서 가져와야 함
            isHost: false,
            tags: ['참가자']
        }))
    ] : [];
                ...
                {postData && (
                    isHost ? (
                        <AccompanyCloseButton
                            ...
                            isClosed={closed} // Pass closed state to the button
                        />
                    ) : (
                        <ApplicationButton
                            ...
                            closed={closed}
                            onLikeToggle={handleLikeToggle}
                            applied={applied} // Pass applied state
                        />
                    )
                )}
