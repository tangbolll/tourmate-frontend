...
// 질문 데이터
const questions = [
    // 계획형(Judging) vs 무계획형(Improvised)
    {
        id: 1,
    ...
    // 모험형(Allocentric) vs 안정형(Psycocentric)
    {
        id: 4,
    ...
    // 지식추구형(Cultural) vs 감성추구형(Emotional)
    {
        id: 7,
...
export default function Question() {
    ...
    const handleTestComplete = (finalAnswers) => {
        // 답변 데이터를 처리하고 결과 페이지로 이동
        console.log('Test completed:', finalAnswers);
        
        // 결과 페이지로 이동 (결과 페이지도 만들어야 함)
    ...
    return (
        ...
        <View style={styles.content}>
            <Text style={styles.questionNumber}>Q. {currentQ.id}</Text>
